...
class QCircuitSimulator:
    def __init__(self, ket, GDepth, DDepth, n, noise):
    ...
    def target_state(self):
        self.register.produce_main_state(self.target)
    ...
    def observe_without_noise(self):
        self.discriminate_state()
        density_mat = self.register.density
        ancprob = np.trace(np.matmul(self.E0, density_mat))
        self.register.initial()  # register state should initial to 0 state
    ...
    def observeg_without_noise(self):
        self.generate_state()
        return self.observe_without_noise()
    ...
    def noise(self, n):
        ...
        if self.noise_type == "Bit_Flip":
            self.bit_flip_noise(n)
        if self.noise_type == "Phase_Flip":
            self.phase_flip_noise(n)
        if self.noise_type == "Bit_Phase_Flip":
            self.bit_phase_flip_noise(n)
        if self.noise_type == "Depolarizing_Channel":
            self.depolarize_noise(n)
