...
class QCircuitSimulator:
    def __init__(self, TDepth, GDepth, DDepth, n, name):
    ...
    def generate_state(self):
        ...
        self.register.change_main_state(gcircuit)  # change register by g
    ...
    def observeg(self):
        self.generate_state()  # 先改变寄存器为经过生成器后的状态，后面observe再改变寄存器为经过判别器后的状态。
        return self.observe()  # 返回的是态经过了生成器判别器后观测到的结果
    ...
    def flip_noise(self):
        dmat = self.register.density  # discriminator's state's density matrix
        if self.noise_name == "Bit_Flip":
            for i in range(1, self.qnum+1):
                x = QG.getX(i, self.qnum)
                y = QG.getY(i, self.qnum)
                z = QG.getZ(i, self.qnum)
                dmat = Pnoise * np.matmul(x, np.matmul(dmat, x)) + (1 - Pnoise) * dmat
        if self.noise_name == "Phase_Flip":
            for i in range(1, self.qnum+1):
                x = QG.getX(i, self.qnum)
                y = QG.getY(i, self.qnum)
                z = QG.getZ(i, self.qnum)
                dmat = Pnoise * np.matmul(z, np.matmul(dmat, z)) + (1 - Pnoise) * dmat
        if self.noise_name == "Bit_Phase_Flip":
            for i in range(1, self.qnum+1):
                x = QG.getX(i, self.qnum)
                y = QG.getY(i, self.qnum)
                z = QG.getZ(i, self.qnum)
                dmat = Pnoise * np.matmul(y, np.matmul(dmat, y)) + (1 - Pnoise) * dmat
        if self.noise_name == "Depolarizing_Channel":
            for i in range(1, self.qnum+1):
                x = QG.getX(i, self.qnum)
                y = QG.getY(i, self.qnum)
                z = QG.getZ(i, self.qnum)
                x1 = np.matmul(x, np.matmul(dmat, x))
                y1 = np.matmul(y, np.matmul(dmat, y))
                z1 = np.matmul(z, np.matmul(dmat, z))
                maxnoise = (dmat + x1 + y1 + z1) / 4
                dmat = Pnoise * maxnoise + (1 - Pnoise) * dmat
        if self.noise_name == "Dephasing_Channel":
            for i in range(1, self.qnum+1):
                x = QG.getX(i, self.qnum)
                y = QG.getY(i, self.qnum)
                z = QG.getZ(i, self.qnum)
