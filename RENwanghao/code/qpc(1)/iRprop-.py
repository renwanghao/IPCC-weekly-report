...
def trace_distance(qcircuit):
    ...
    a = lin.sqrtm(np.matmul(np.asmatrix(tar_state - gen_state).getH(), (tar_state - gen_state)))
    td = np.trace(a) * 0.5
    return td.real
...
def iteration(qcircuit, m):
    ...
    axs1.plot(range(len(y2)), y2)
    axs1.set_xlabel('Epoch')
    axs1.set_ylabel('fidelity between real and fake states')
    axs2.plot(range(len(y1)), y1)
    axs2.set_xlabel('Epoch')
    axs2.set_ylabel('tracedistance between real and fake states')
    plt.savefig('./test3.png')
    plt.show()


def simulate():
    cir = QC.QCircuitSimulator(2, 2, 2, 3, "Depolarizing_Channel")
    # noise_name = Bit_Flip,Phase_Flip(Dephasing_Channel), Bit_Phase_Flip, Depolarizing_Channel,
