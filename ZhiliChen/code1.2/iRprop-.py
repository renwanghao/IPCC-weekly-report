...
def iteration(qcircuit):
    ...
    for i in range(0, 10):
        ...
        for i in range(0, 5):
            ...
            dis_der1 = dis_der2[:]
        fidelity1 = fidelity(qcircuit)
        print(fidelity1)
        fidelities.append(fidelity1)
        np.savetxt('fidelities', fidelities)
...
def simulate():
    ...
    iteration(cir)
    with open('cir', 'wb') as file:
        pickle.dump(cir, file)
    # cir1 = cir
    # with open('cir', 'rb') as file:
    #     model = pickle.load(file)
    # cir2 = model
    # print(cir, cir1, cir2)
    a = cir.get_tar_state()
    np.savetxt('target_state', a)
    print(a)
    b = cir.get_gen_state()
    print(b)
