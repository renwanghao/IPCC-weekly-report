...
def fidelity(qcircuit):
    ...
    gen_state = qcircuit.main_register.state
    qcircuit.register_initial()
    qcircuit.target_state()
    tar_state = qcircuit.main_register.state
    qcircuit.register_initial()
...
def trace_distance(qcircuit):
    qcircuit.generate_state()
    gen_state = np.matmul(qcircuit.main_register.state, qcircuit.main_register.state.T.conj())
    qcircuit.register_initial()
    qcircuit.target_state()
    tar_state = np.matmul(qcircuit.main_register.state, qcircuit.main_register.state.T.conj())
    qcircuit.register_initial()
...
def iteration(qcircuit):
    ...
    dis_der1 = [0] * len(dis_para)
    dis_step = [InitStep] * len(dis_para)
    # execute 400 iterations, each iteration trains generator once and trains discriminator 5 times
    for i in range(0, 400):
        ...
        y1.append(trace_distance(qcircuit))
        y2.append(fidelity(qcircuit))
    x = np.arange(0, 400, 1)
...
def simulate():
    cir = QC.QCircuitSimulator(1, 1, 1, 2)
    iteration(cir)
    cir.target_state()
    print(cir.main_register.state)
    cir.register_initial()
    cir.generate_state()
    print(cir.main_register.state)
    cir.discriminate_state()
    print(cir.register.state)
    cir.register_initial()
