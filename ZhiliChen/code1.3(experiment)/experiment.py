...
def save_cir(type, i, cir):
    file_name = "./experiment/3bit/" + str(i) + "/" + type + "/cir"
    c = pickle.dumps(cir)
...
for i in range(1, 11):
    cir = QC.QCircuitSimulator(2, 2, 2, 3, "Without_Noise")# noise_name = Bit_Flip,Phase_Flip, Bit_Phase_Flip, Depolarizing_Channel
    save_cir("init", i, cir)
